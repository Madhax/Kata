...
public:
    int countPairs(vector<int>& nums, int low, int high) {
        sort(all(nums));
        int n = nums.size();
        auto ass = [&](int*p,int*q,int x,int lt) {
            if (p==q) return 0;
            int ans = 0;
            for (int bit = 30; bit >= 0; --bit) {
                if (lt&(1<<bit)) {
                    auto A = (x^(lt&~(1<<bit)))&~((1<<bit)-1);
                    auto B = (x^(lt&~(1<<bit)))|((1<<bit)-1);
                    auto lo = lower_bound(p,q,A);
                    auto hi = upper_bound(p,q,B);
                    ans += hi-lo;
                }
            }
            return ans;
        ...
        int ans = 0;
        for (int i = 0; i < n; ++i) {
            auto p = &nums[i+1], q = &nums[n];
            int plus = ass(p, q, nums[i], high+1);
            int minus = ass(p, q, nums[i], low);
            ans += plus-minus;
        }
        return ans;
    }
